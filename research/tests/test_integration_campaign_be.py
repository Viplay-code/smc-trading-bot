"""
research/tests/test_integration_campaign_be.py — Validación ESTRUCTURAL de
scripts/integration_campaign_be.py sobre datos sintéticos.
...
(gestion_campaign_be_results.csv/gestion_campaign_session_results.csv) —
prueba que _verify_against y verify_anchor_dual_role detectan
coincidencias y mismatches correctamente, que BE_ANCHOR (1.0) SÍ aparece
en CANDIDATES (rol dual, mismo diseño que I1-activation), que
...
Ejecutar:
    python -m research.tests.test_integration_campaign_be  (o con pytest)
"""
...
import scripts.integration_campaign_be as camp
...
# --------------------------------------------------------------------------- #
def test_be_candidates_frozen_from_paso_0_no_pruning():
    ok = camp.BE_CANDIDATES == (0.25, 0.5, 1.0, 1.5, 2.0)
    return _p("BE_CANDIDATES == conjunto no-dominado del Paso 0, sin poda (5 valores)", ok)


def test_anchor_is_in_candidates():
    ok = any(c.startswith(f"{camp.BE_ANCHOR} |") for c in camp.CANDIDATES)
    return _p(f"BE_ANCHOR (1.0) SÍ aparece en CANDIDATES (rol dual) ({camp.CANDIDATES})", ok)


def test_candidates_size_matches_grid():
    ok = len(camp.CANDIDATES) == len(camp.BE_CANDIDATES) * len(camp.SESSION_LABELS) == 10
    return _p("CANDIDATES tiene exactamente 5 be x 2 sesiones = 10 combinaciones", ok)
...
# --------------------------------------------------------------------------- #
def _fake_row(asset, year, be, session, role, pf, n_trades=48, n_entries=50):
    ...
    return {
        ...
        "candidate": camp._label(be, session), "exit_config": camp.EXIT_CONFIG_LABEL,
...
def test_verify_anchor_dual_role_passes_when_identical():
    ctrl = [_fake_row("BTCUSDT", 2022, 1.0, "control_8h", "control", pf=1.0)]
    cand = [_fake_row("BTCUSDT", 2022, 1.0, "control_8h", "candidate", pf=1.0)]
    ok = True
    try:
...
def test_verify_anchor_dual_role_raises_on_pf_mismatch():
    ctrl = [_fake_row("BTCUSDT", 2022, 1.0, "control_8h", "control", pf=1.0)]
    cand = [_fake_row("BTCUSDT", 2022, 1.0, "control_8h", "candidate", pf=1.05)]
    ok = False
    try:
        camp.verify_anchor_dual_role(ctrl, cand)
    except AssertionError as e:
...
def test_verify_anchor_dual_role_raises_on_n_trades_mismatch():
    ctrl = [_fake_row("BTCUSDT", 2022, 1.0, "control_8h", "control", pf=1.0, n_trades=48)]
    cand = [_fake_row("BTCUSDT", 2022, 1.0, "control_8h", "candidate", pf=1.0, n_trades=47)]
    ok = False
    try:
        camp.verify_anchor_dual_role(ctrl, cand)
    except AssertionError as e:
...
def test_run_control_checks_pass_when_reference_matches_exactly():
    ...
    ref_h23 = _series_from_row(row_c8)
    ref_sess = _series_from_row(row_15)

    orig_h23 = camp._h23_reference
    orig_sess = camp._session_reference
    camp._h23_reference = lambda asset, year: ref_h23
...
def test_run_control_checks_raises_when_reference_wrong():
    ...
    camp._h23_reference = lambda asset, year: wrong_ref
...
def test_full_pipeline_end_to_end_on_synthetic_slice():
    ...
    def _matching_ref(asset, year, session_label):
        ...
        _, m = camp._run_be(frame, cfg, entries, camp.BE_ANCHOR)
        d = {k: m[k] for k in camp._CHECK_FIELDS}
        d["n_trades"] = len(backtest.run_config(frame, entries, camp._exit_cfg(camp.BE_ANCHOR), cfg))
        return pd.Series(d)

    camp._h23_reference = lambda asset, year: _matching_ref(asset, year, "control_8h")
...
def test_summarize_decision_after_filtering_control_rows():
    rows = [
        _fake_row("BTCUSDT", 2022, 0.5, "dcv1_activo_15h", "candidate", pf=1.6),
        _fake_row("BTCUSDT", 2023, 0.5, "dcv1_activo_15h", "candidate", pf=1.55),
        _fake_row("BTCUSDT", 2022, 0.5, "control_8h", "candidate", pf=0.9),
        _fake_row("BTCUSDT", 2023, 0.5, "control_8h", "candidate", pf=0.8),
    ]
    df = camp.results_to_frame(rows)
    ...
    return _p("summarize_decision distingue sesiones distintas del mismo be sin mezclarlas", ok)
...
def test_run_blind_test_requires_frozen_candidate():
    ...
    try:
        ...
        pass
    try:
        camp.run_blind_test(candidate="0.9 | control_8h")  # no está en CANDIDATES
        ok = False
    except ValueError:
        pass
...
def test_run_blind_test_parses_candidate_string():
    sliced, errs = _build_sliced_for(seed=7)
    if sliced.empty:
        return _p("run_blind_test parsea 'be | session' (slice vacío)", False)
    orig = trigger_camp.load_asset_year
    trigger_camp.load_asset_year = lambda asset, year: sliced
    try:
...
ALL_TESTS = [
    test_be_candidates_frozen_from_paso_0_no_pruning,
    test_anchor_is_in_candidates,
...
def main():
    print("scripts/integration_campaign_be — validación estructural sobre datos sintéticos\n")
