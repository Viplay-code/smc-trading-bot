#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""scripts/integration_campaign_activation.py — Fase de Integración (I1),
bloque `activation`: segunda prueba conjunta de sesión × un parámetro de
Gestión de la familia H2, bajo Bias=A/Trigger=T1_ema_cross/Entry=
C_market_close/atr_mult=1.5/be=1.0R/distance=1.0R fijos (anclas V3-A de
H2.2 — NO los hallazgos de H2.1/H2.3, misma regla de no-promoción).

Contexto: sigue a I1-distance (cerrada 2026-08-05, commit `8688020`,
falsificada — ninguna combinación distance×sesión superó los 4 gates,
0/36 filas candidatas alcanzaron siquiera PF>=1.50 en un solo año). Este
bloque prueba la misma pregunta con `activation` en vez de `distance`.

Hipótesis (I1-activation) a falsificar: existe al menos una combinación
(`activation` ∈ conjunto no-dominado del Paso 0, sesión ∈ {control_8h,
dcv1_activo_15h}) que, con `be`=1.0R/`distance`=1.0R fijos en su ancla
...
ACTIVATION_CANDIDATES = {1.25, 1.5, 2.0, 2.5, 3.0}, los 5 valores
probados en H2.2, SIN PODA (ninguno quedó dominado en todos los contextos
por otro — a diferencia de `distance`, donde el ancla quedó excluida, acá
`activation`=2.0 SÍ integra el conjunto no-dominado). Cualquier cambio
futuro exige repetir formalmente el Paso 0 y aprobar un nuevo contrato.

Rol dual del ancla (`activation`=2.0, decisión acordada 2026-08-06): se
computa DOS VECES dentro de la misma corrida — una vez como control
(Fase A, ver abajo) y una vez como candidato normal dentro del barrido de
la Fase B (participa en CANDIDATES/summarize_decision()/--blind igual que
cualquier otro valor). Excluirlo de la Fase B habría introducido un
criterio de exclusión ajeno a la dominación Pareto del Paso 0 — se
prefirió preservar "el espacio experimental lo define únicamente el Paso
0" sin excepciones, aceptando la redundancia mínima de recalcular el
mismo valor dos veces. Esa redundancia se aprovecha como verificación
extra: `verify_anchor_dual_role` exige que la fila role='control' de
activation=2.0 y su fila role='candidate' equivalente (misma sesión)
coincidan EXACTO en TODAS las métricas que la campaña publica (pf, wr,
exp_r, avg_win, avg_loss, total_r, max_dd, freq, be, reason_stop,
reason_timeout, n_entries, n_trades) — no solo en el resultado final
(pf/gate_pass) — porque ambas filas son, por construcción, la misma
computación alcanzada por dos caminos de código distintos (run_control_rows
vs run_asset_year_experimental); cualquier divergencia en cualquier campo
indica un bug interno de separación de roles, no un desvío respecto de
datos históricos (eso ya lo cubre la Fase A por separado).
...
verificación):
  - Fase A (verificación de integridad, 12 verificaciones independientes
    ...
    de control (`activation`=2.0 bajo control_8h y bajo dcv1_activo_15h) y
    las compara EXACTO (sin tolerancia, mismo redondeo determinístico de
    backtest.metrics()) contra:
      * control_8h + activation=2.0 -> gestion_campaign_activation_
        results.csv (fila candidate=2.0, exit_config="H2.2 anchor
        (be=1.0R/distance=1.0R, activation variable)", ya publicada,
        H2.2) — 6 verificaciones, una por (activo, año).
      * dcv1_activo_15h + activation=2.0 -> gestion_campaign_session_
        results.csv (fila candidate="dcv1_activo_15h", exit_config=
        "V3-A (1R/2R/1R)", ya publicada — MISMA referencia que usó
        I1-distance, porque el paquete V3-A completo be=1.0/
        activation=2.0/distance=1.0 es el mismo ancla en ambos bloques)
        — 6 verificaciones más, una por (activo, año).
  ...
  - Fase B (barrido experimental): arranca SOLO si la Fase A completa sin
    ...
    combinaciones candidatas (`activation` x sesión) por activo/año
    (incluye activation=2.0 x 2 sesiones, por su rol dual). Tras
    calcularse, `verify_anchor_dual_role` compara esas 6 filas (3
    activos x 2 años, sesión=control_8h únicamente por simplicidad de
    contraste directo — ver función) contra las filas de control ya
...
C_market_close, `atr_mult`=1.5 (ancla H1), `be`=1.0R/`distance`=1.0R
(anclas V3-A de H2.2 — NO los hallazgos de H2.1/H2.3), `atr_period`=14,
`max_hold`=20, `risk`=0.005, una posición a la vez, datasets dc_v1
...
`candidate` = f"{activation} | {session_label}" para que
`summarize_decision()` (reutilizada SIN modificar) no mezcle sesiones
distintas bajo el mismo `activation` en su agrupamiento por (asset,
candidate, exit_config). `summarize_decision()` opera EXCLUSIVAMENTE
sobre filas `role="candidate"` — `role="control"` existe únicamente para
la verificación de integridad de la Fase A (y, acá, también para la
verificación de rol dual) y nunca se pasa a esa función, nunca aparece en
`CANDIDATES`, nunca es elegible para `--blind`.
...
research/tests/test_integration_campaign_activation.py.

Uso (desde la raíz del repo, con data/raw/ poblado):
    python scripts/integration_campaign_activation.py              # Fase 1+2 (incluye Fase A+B internas)
    python scripts/integration_campaign_activation.py --blind        # Fase 3: 2024 ciego (requiere --candidate)
"""
...
# --------------------------------------------------------------------------- #
BE_ANCHOR = 1.0          # V3-A / ancla de H2.2, congelado
DISTANCE_ANCHOR = 1.0    # V3-A / ancla de H2.2, congelado — NO el hallazgo de H2.1
ATR_MULT_ANCHOR = 1.5    # ancla de H1, congelado
ACTIVATION_ANCHOR = 2.0  # V3-A — control de integridad Y candidato (rol dual, ver docstring)

# Congelado por el Paso 0 (2026-08-04) — dominación Pareto sobre
# gestion_campaign_activation_results.csv. SIN poda: los 5 valores
# probados en H2.2 sobreviven. Este script NO recalcula dominación.
ACTIVATION_CANDIDATES = (1.25, 1.5, 2.0, 2.5, 3.0)
...
CANDIDATES = tuple(
    f"{a} | {s}" for a in ACTIVATION_CANDIDATES for s in SESSION_LABELS
)
EXIT_CONFIG_LABEL = "I1-activation (be=1.0R/distance=1.0R, activation & session variable)"
...
_H22_REF_PATH = "gestion_campaign_activation_results.csv"
_H22_EXIT_CONFIG = "H2.2 anchor (be=1.0R/distance=1.0R, activation variable)"
...
def _label(activation: float, session_label: str) -> str:
    return f"{activation} | {session_label}"
...
def _row(asset: str, year: int, activation: float, session_label: str,
          entries: list[dict], trades: pd.DataFrame, m: dict | None, role: str) -> dict:
    return {
        "asset": asset, "year": year, "role": role, "session": session_label,
        "candidate": _label(activation, session_label), "exit_config": EXIT_CONFIG_LABEL,
...
# (12 verificaciones independientes: 6 contra gestion_campaign_activation_    #
# results.csv + 6 contra gestion_campaign_session_results.csv)               #
# --------------------------------------------------------------------------- #
def _h22_reference(asset: str, year: int) -> pd.Series:
    df = pd.read_csv(_H22_REF_PATH)
    df["candidate_f"] = df["candidate"].astype(float)
    match = df[(df["asset"] == asset) & (df["year"] == year)
               & (df["candidate_f"] == ACTIVATION_ANCHOR)
               & (df["exit_config"] == _H22_EXIT_CONFIG)]
    if match.empty:
        raise AssertionError(
            f"No se encontró fila de referencia H2.2 para {asset}/{year}/activation="
            f"{ACTIVATION_ANCHOR} en {_H22_REF_PATH} — no se puede verificar la Fase A."
        )
    return match.iloc[0]
...
def run_control_rows(asset: str, year: int) -> list[dict]:
    ...
    una por sesión, con activation=ACTIVATION_ANCHOR."""
    rows = []
    for session_label in SESSION_LABELS:
        frame, cfg, entries = _entries_for_session(asset, year, session_label)
        trades, m = _run_activation(frame, cfg, entries, ACTIVATION_ANCHOR)
        rows.append(_row(asset, year, ACTIVATION_ANCHOR, session_label, entries, trades, m, role="control"))
    return rows
...
def run_control_checks(asset: str, year: int) -> list[dict]:
    ...
    exitosamente contra los CSV históricos ya publicados (H2.2 y campaña de
    ...
    ref_h22 = _h22_reference(asset, year)
    _verify_against(row_c8["metrics"], row_c8["n_trades"], ref_h22,
                     f"{asset}/{year} control_8h vs H2.2 ({_H22_REF_PATH})")
...
def run_asset_year_experimental(asset: str, year: int) -> list[dict]:
    ...
    for session_label in SESSION_LABELS:
        ...
        for activation in ACTIVATION_CANDIDATES:
            ...
            rows.append(_row(asset, year, activation, session_label, entries, trades, m, role="candidate"))
    return rows
...
def verify_anchor_dual_role(control_rows: list[dict], experimental_rows: list[dict]) -> None:
    ...
    de activation=ACTIVATION_ANCHOR coincida EXACTO en TODOS los campos
    publicados (_FULL_FIELDS, no solo pf/gate_pass) con su fila
    role='candidate' equivalente generada en la Fase B — ambas son, por
    construcción, la misma computación alcanzada por caminos de código
    distintos. Cualquier divergencia revienta con AssertionError."""
    anchor_label_by_session = {s: _label(ACTIVATION_ANCHOR, s) for s in SESSION_LABELS}
    ...
    for ctrl in control_rows:
        ...
        if mismatches:
            raise AssertionError(
                ...
                f"la fila de control y la fila candidata de activation="
                f"{ACTIVATION_ANCHOR} deberían ser IDÉNTICAS en todos los campos "
...
def run_blind_test(assets: tuple[str, ...] = ASSETS,
    ...
    "{activation} | {session}") ya congelado tras decidir con 2022+2023. No
    ...
    activation_str, session_label = (p.strip() for p in candidate.split("|"))
    activation = float(activation_str)
    results: list[dict] = []
    for asset in assets:
        frame, cfg, entries = _entries_for_session(asset, BLIND_YEAR, session_label)
        trades, m = _run_activation(frame, cfg, entries, activation)
        results.append(_row(asset, BLIND_YEAR, activation, session_label, entries, trades, m, role="candidate"))
    return results
...
def print_report(df: pd.DataFrame) -> None:
    print(f"\n{'='*100}\n  I1 — bloque activation: sesión x activation bajo Bias=A/Trigger=T1_ema_cross/"
    ...
    print(f"\n{'-'*100}\n  Fase A — filas de control (integridad verificada OK contra H2.2 y campaña de sesión):\n{'-'*100}")
    print(control[cols].to_string(index=False) if not control.empty else "  (sin filas de control)")

    candidate = df[df["role"] == "candidate"]
    print(f"\n{'-'*100}\n  Fase B — barrido experimental (incluye activation=2.0 por su rol dual, "
...
def print_decision(decision: pd.DataFrame) -> None:
    ...
    for asset, g in decision.groupby("asset"):
        ...
        if survivors.empty:
            print(f"\n  {asset}: ninguna combinación (activation, sesión) en {CANDIDATES} demuestra "
                  f"evidencia suficiente para cumplir los 4 gates de FRAMEWORK.md en 2022+2023 — "
                  f"I1-activation queda sin evidencia suficiente para este activo.")
...
def main() -> None:
    ...
    parser.add_argument("--candidate", choices=CANDIDATES, default=None,
                         help="candidato ya congelado ('{activation} | {sesión}') para --blind")
    ...
    print_report(df)
    out_path = "integration_campaign_activation_results.csv"
    ...
    if not args.blind:
        ...
        decision_path = "integration_campaign_activation_decision.csv"
