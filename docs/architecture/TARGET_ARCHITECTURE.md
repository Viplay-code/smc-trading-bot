...
**Estado:** Referencia oficial de arquitectura (v12, sobre Architecture Baseline v1)
...
| Simulador de gestión genérico | **Extraído (Fase 4/C2), con SEGUNDO mecanismo registrado (Fase 5, 2026-09-02)** — `simulate_v3`/`EXIT_CONFIGS` MOVIDOS a `research/simulate.py`, sin cambiar una línea de su lógica. `research.EXIT_CONFIGS` ahora tiene 3 entradas: `"V3-A (1R/2R/1R)"`, `"V3-B (0.75R/1.5R/0.75R)"` (sin cambios) y `"Raw"` (nueva, Fase 5 — `{"be": inf, "activation": inf, "distance": 0.0}`, copiada exacta desde `scripts/gestion_espacio6_raw_campaign.py`). Raw NO es una función de simulación nueva — usa el MISMO `simulate_v3` que V3-A, solo con otro `exit_cfg` (verificado por identidad de objeto). `run_config`/`COST_PER_TRADE` siguen sin moverse (mismo motivo que Fase 4). Efecto colateral documentado (no silencioso): scripts legacy que iteran `EXIT_CONFIGS.items()` calcularán una fila "Raw" adicional si se re-ejecutan — no cambia ningún valor ya publicado, verificado que no rompe ningún test (523/523 pasan) | Extraído a `research/simulate.py` — motor completo, con 2 mecanismos de Gestión ya registrados en `research/runner.py::MANAGEMENT_LAYERS` |
...
| Experiment Runner | Manual (correr script, leer consola) para las ~25 campañas legacy. **`research/runner.py` EXISTE desde Fase 2, ENDURECIDO en Fase 3, con motor extraído en Fase 4, y AHORA CON 2 MECANISMOS DE GESTIÓN (Fase 5, 2026-09-02)**: `MANAGEMENT_LAYERS = {"V3-A": ..., "Raw": ...}`, resueltos vía el registro (`MANAGEMENT_LAYERS[management_name]`), sin ningún `if`/`elif` por nombre (verificado por inspección AST). Interfaz común `ManagementFn = Callable[[frame, entries, cfg], DataFrame]` — `cost_per_trade` se removió deliberadamente de esa interfaz (pertenece a la capa de ejecución, no a la de Gestión; el parcheo de `backtest.COST_PER_TRADE` ahora vive una sola vez en `run()`, no repetido por mecanismo). **Equivalencia exacta (sin redondeo) demostrada contra las 6/6 celdas completas de `gestion_campaign_session_results.csv` (V3-A) Y contra las 6/6 celdas de `gestion_espacio6_raw_results.csv` (Raw)**, más equivalencia de `TradeRecord` completo (Raw: runner vs. ejecución legacy fresca, sin artefacto histórico persistido — nunca existió uno). `research.TradeRecord` (declarado en Fase 1) ya tiene consumidor real: `run(experiment, include_trades=True)`. Dependencia `research → scripts` sigue eliminada (Fase 3). Sin barrido multi-activo/año, sin orquestación train→validate→blind, sin selección de "ganador" | Automatizado en `research/runner.py`, con manifiesto versionado — **2 mecanismos de Gestión soportados, `TradeRecord` conectado; generalización a más mecanismos/activos/años pendiente** |
...
| **D** | `research/runner.py`: barrido completo 2022→2023→2024 con disciplina de blind set | A, B, C1, C2, C3 | **Parcial, endurecido, con 2 mecanismos (MVP, Fase 2-5, 2026-09-02).** `research/runner.py` EXISTE — `run(experiment)` determinista, con equivalencia exacta (sin redondeo, verificada a nivel de trade record completo) demostrada contra **6/6 celdas completas** de V3-A (`gestion_campaign_session_results.csv`) Y **6/6 celdas completas** de Raw (`gestion_espacio6_raw_results.csv`). Dependencia `research → scripts` eliminada (Fase 3). El motor de simulación (`simulate_v3`/`EXIT_CONFIGS`) vive en `research/simulate.py` (Fase 4/C2) — `MANAGEMENT_LAYERS` (Fase 5) ya resuelve 2 mecanismos (`V3-A`, `Raw`) por registro, sin `if`/`elif`. `research.TradeRecord` conectado (`include_trades=True`). Alcance deliberadamente angosto (ver §5, fila "Experiment Runner") — NO implementa todavía barrido multi-(activo,año), orquestación train→validate→blind, ni mecanismos que requieran una función de simulación propia (E1/TP fijo/V3A+TP) |
...
| v11 | Fase 4 — C2: extracción del motor de simulación (2026-09-02). Auditoría previa identificó dos riesgos reales de dependencia circular/monkeypatch roto (documentados en `research/simulate.py`), resueltos por diseño: `simulate_v3`/`EXIT_CONFIGS` MOVIDOS a `research/simulate.py` sin cambiar una línea de lógica (precedencia intrabar, SL, BE, trailing, timeout, costos, sizing, outputs preservados — evidencia histórica intacta); `run_config`/`COST_PER_TRADE` DELIBERADAMENTE NO movidos (permanecen en `backtest.py`, re-exportados con el mismo patrón de wrapper de v8/v10) porque moverlos rompería, silenciosamente, los monkeypatches de tests ya existentes (`backtest.simulate_v3 = ...`, `backtest.COST_PER_TRADE = ...`) — `research.simulate_v3` sigue leyendo el costo real desde `backtest.COST_PER_TRADE` vía un import LOCAL A LA FUNCIÓN (no a nivel de módulo), verificado sin dependencia circular en los 3 órdenes de import posibles, en subprocesos frescos. Equivalencia exacta reconfirmada (6/6 celdas + trade record completo, sin redondeo) tras la extracción. `research/runner.py::MANAGEMENT_LAYERS["V3-A"]` actualizado para referenciar `research.EXIT_CONFIGS`/`research.simulate_v3` explícitamente. Ningún mecanismo de Gestión nuevo, ningún cambio a `bot.py`/`FRAMEWORK.md`/parámetros/gates. §5 y §6.1 actualizados. |
| v12 (este documento) | Fase 5 — segundo mecanismo de Gestión ("Raw") + `TradeRecord` con consumidor real (2026-09-02). Auditoría previa confirmó que Raw no requiere una función de simulación propia (usa `simulate_v3` con otro `exit_cfg`) — sin bloqueo arquitectónico. `research.EXIT_CONFIGS` ganó la entrada canónica `"Raw"` (`be`/`activation`=inf, `distance`=0.0, copiada exacta desde el script legacy de Espacio 6; efecto colateral documentado sobre scripts legacy que iteran `EXIT_CONFIGS.items()`, sin romper ningún test). `research/runner.py::MANAGEMENT_LAYERS` ampliado a `{"V3-A": ..., "Raw": ...}`, ambos construidos por una única fábrica (`_make_run_config_management`), sin ningún `if`/`elif` de selección manual (verificado por inspección AST). Decisión de interfaz: `cost_per_trade` removido de `ManagementFn` (pertenece a la capa de ejecución, no a la de Gestión) — el parcheo de `backtest.COST_PER_TRADE` se centralizó en `run()`. `research.TradeRecord` (declarado en Fase 1) conectado por primera vez: `run(experiment, include_trades=True)` devuelve `(ExperimentResult, list[TradeRecord])`, construido desde el mismo `trades` que ya alimentó las métricas — sin alterarlas, sin fabricar `exit_price` (ausente en `simulate_v3`, queda `None`), con `entry_price` derivado determinísticamente vía join contra `entries`. Equivalencia exacta reconfirmada: V3-A 6/6 (sin regresión) + Raw 6/6 (nueva, contra `gestion_espacio6_raw_results.csv`) + TradeRecord de Raw (runner vs. ejecución legacy fresca, sin artefacto histórico persistido — nunca existió uno, documentado explícitamente). Suite completa: 523/523 tests, sin regresiones. Ningún mecanismo que requiera una función de simulación propia (E1/TP fijo/V3A+TP), ningún cambio a `simulate_v3`/`bot.py`/`FRAMEWORK.md`/parámetros/gates. §5 y §6.1 actualizados. |
